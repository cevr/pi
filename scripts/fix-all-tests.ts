...
function getExports(sourceFile: string): string[] {
	...
	for (const match of content.matchAll(/export\s+(?:async\s+)?function\s+(\w+)/g)) exports.push(match[1]!);
	for (const match of content.matchAll(/export\s+(?:const|let|var)\s+(\w+)/g)) exports.push(match[1]!);
	for (const match of content.matchAll(/export\s+class\s+(\w+)/g)) exports.push(match[1]!);
	for (const match of content.matchAll(/export\s+(?:interface|type)\s+(\w+)/g)) exports.push(match[1]!);
	for (const match of content.matchAll(/export\s+default\s+(?:function|class)\s+(\w+)/g)) exports.push(match[1]!);
...
function fixTestFile(testFile: string): boolean {
	...
	for (const imp of importLines) {
		for (const match of imp.matchAll(/\{([^}]+)\}/g)) {
			for (const name of match[1]!.split(",")) {
				alreadyImported.add(name.trim().split(/\s+as\s+/).pop()!.trim());
			}
		}
	...
	const bunTestAll = ["describe", "expect", "it", "test", "beforeEach", "beforeAll", "afterEach", "afterAll", "mock", "spyOn"];
	...
	if (neededFromSource.length > 0) {
		...
		const existingSourceImportIdx = updatedImportLines.findIndex((l) => l.includes(`from "${relPath}"`));
		if (existingSourceImportIdx >= 0) {
			...
			if (match) {
				...
				updatedImportLines[existingSourceImportIdx] = `import { ${[...names].join(", ")} } from "${relPath}";`;
			}
		} else {
