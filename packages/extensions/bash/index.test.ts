...
describe("bash tool output formatting", () => {
  ...
  describe("large output (truncation)", () => {
    it("shows head + tail for large output", async () => {
      const result = await execute(`python3 -c "for i in range(1, 201): print(f'line {i}')"`);
      const text = result.content[0].text;
  ...
  describe("reversion guards", () => {
    ...
    it("keeps head lines before tail lines in truncated output", async () => {
      const result = await execute(`python3 -c "for i in range(1, 151): print(f'line {i}')"`);
      const text = result.content[0].text;
  ...
  describe("edge cases", () => {
    ...
    it("handles many short lines", async () => {
      const result = await execute(`python3 -c "for _ in range(500): print('x')"`);
  ...
  describe("background commands", () => {
    ...
    it("kills background commands during cleanup", async () => {
      const result = await execute(`python3 -c "import time; time.sleep(60)" &`);
