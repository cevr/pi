...
function createTestDeps() {
  ...
  const createMentionProvider = mock((_baseProvider, _context) => mentionProvider);
  ...
  return {
    deps: {
      ...
      clearSessionMentionCache: clearSessionMentionCacheMock as typeof clearSessionMentionCache,
      clearCommitIndexCache: clearCommitIndexCacheMock as typeof clearCommitIndexCache,
...
describe("mentions extension", () => {
  ...
  it("registers an editor autocomplete contributor", () => {
    const { deps, registerAutocompleteContributor, createMentionProvider } = createTestDeps();
  ...
  it("resolves mentions on input and injects hidden context only when text is non-empty", async () => {
    const { deps, resolveMentionsMock, renderResolvedMentionsTextMock } = createTestDeps();
    const mentionsExtension = createMentionsExtension(deps);
    const harness = createMockExtensionApiHarness();
    ...
    resolveMentionsMock.mockResolvedValue([resolvedSessionMention, unresolvedMention]);
    ...
    expect(renderResolvedMentionsTextMock).toHaveBeenCalledWith([resolvedSessionMention]);
    ...
    await expect(contextHandler({ messages: baseMessages })).resolves.toEqual({
      messages: [
        ...
        expect.objectContaining({
          ...
          content: "resolved mention context:\n@session/alpha1234\tsession\talpha1234",
          display: false,
        }),
      ],
    });
    ...
    await expect(
      inputHandler({ source: "user", text: "open @session/alpha1234" }, { cwd: "/repo/app" }),
    ).resolves.toEqual({ action: "continue" });
  ...
  it("clears adapter state on agent_end, session_start, and session_switch", async () => {
    ...
    const primeState = async () => {
      await inputHandler({ source: "user", text: "@session/alpha1234" }, { cwd: "/repo/app" });
      await expect(contextHandler({ messages: baseMessages })).resolves.toEqual({
        messages: [...baseMessages, expect.objectContaining({ customType: CUSTOM_TYPE })],
      });
    };
